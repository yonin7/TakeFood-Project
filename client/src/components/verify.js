...
const Verify = (props) => {
  ...
  const { user, loading: loading1 } = useContext(AuthContext);
  ...
  const handleSubmit = async (e) => {
    ...
    if (code === '') {
      ...
      try {
        const res = await baseURL.post('/verify', { code }, config);
        setLoading(false);
        document.write(
        ...
        setStatus({ val: 1, type: 'red', msg: e.response.data });
        let id = setTimeout(() => {
          setStatus({ val: 0, type: '', msg: '' });
          clearTimeout(id);
        }, [3000]);
        setLoading(false);
      }
  ...
  const resend = async () => {
    ...
    try {
      setLoading(true);
      const res = await baseURL.post('/resend', { id, email, name }, config);
              ...
              <div className="forget1  animate__animated animate__backInUp">
                <p>
                  <span style={{ fontFamily: 'Poppins', fontSize: '0.95rem' }}>
                    {' '}
                    <MailOutlineIcon /> Enter the 6-digit code we sent to your
